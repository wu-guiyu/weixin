...
var newOpenId = "";
/**
 * 两种支付方式；如果没有优惠券，默认直接调用微信支付
 * 
 * @returns
 */
function bookVerifyCar(openId) {
	if (!verifyCar(openId)) {
		return;
	}
	newOpenId = openId;
	...
	layer.open({
		...
		btn : ['微信支付', '优惠券支付' ],
		yes : function(index) {
			doPrePayId(newOpenId);
		},
		no : function(index) {
			couponBook(newOpenId);
		}
	});
//	layer.confirm('', {
//		title : [ '请选择预约支付方式', 'font-size:14px' ],
 ...
 * @returns
 */
function couponBook(openId) {
	var index = showLoading("正在预约中……");
	$.ajax({
		type : 'POST',
		...
		success : function(res) {
			...
			if (res.status == 1) { // 如果用户有优惠券，不走微信支付流程，直接保存订单，并修改用户优惠券表中的数据
				// 删除用户优惠券表中的数据
				var data = res.data;
				if(data != null && data.length > 0){
					var cp_owner = data[0].cp_owner;
					var cp_start_time = data[0].cp_start_time;
					var cp_end_time = data[0].cp_end_time;
					deleteCouponByUse(cp_owner, cp_start_time, cp_end_time);
				}
				// 直接保存订单
				openidValue();
				// 预约成功
				layerTip("恭喜您，预约成功~……");
			} else { // 如果用户没有优惠券，走支付流程
				layerTip("您当前没有优惠券，请重新选择支付方式……");
			}
		},
		error : function() {
			layerTip("未知错误，请重新下单~");
...
/**
 * 提交预约审车订单成功后，删除已经使用的优惠券记录
 * 
 * @returns
 */
function deleteCouponByUse(openId, startTime, endTime) {
	$.ajax({
		type : 'POST',
		url : getRootPath() + "userCoupon/deleteByIdAndTimes",
		data : {
			"id" : openId,
			"startTime" : startTime,
			"endTime" : endTime
		...
		success : function(res) {
			if (res.status == 1) { // 如果用户有优惠券，不走微信支付流程，直接保存订单，并修改用户优惠券表中的数据
				layerTip("已通过优惠券成功抵扣所需缴纳的定金，下单成功~");
			} else { // 如果用户没有优惠券，走支付流程
				layerTip("未能通过优惠券方式抵扣定金，请重新支付定金");
			}
		},
		error : function() {
			layerTip("未知错误");
...
function doPrePayId(openId) {
	...
	var index = showLoading("预约支付中，请稍等片刻……");
	// 获取预支付id
	$.ajax({
		type : 'POST',
