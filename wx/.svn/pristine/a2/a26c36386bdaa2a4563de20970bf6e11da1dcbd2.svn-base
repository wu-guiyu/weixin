...
public class TemplateMessageManager {
	...
	/**
	 * @category 海报助力成功后，发送消息给客户
	 * @param openid
	 * @param name
	 * @param time
	 * @throws Exception
	 */
	public static void hbTemplateMessageToCustomer(String openid) throws Exception {
		String at = Util.getAccessToken();
		String url = "https://api.weixin.qq.com/cgi-bin/message/template/send?access_token=ACCESS_TOKEN";
		url = url.replace("ACCESS_TOKEN", at);
		JSONObject jsonObject = new JSONObject();
		JSONObject data = new JSONObject();
		data.put("first", getJSONObject("您好，代金劵已发放：", COLOR));
		data.put("keyword1", getJSONObject("审车优惠活动", COLOR));
		data.put("keyword2", getJSONObject("10人", COLOR));
		data.put("remark", getJSONObject("请您查收，在规定时间使用", COLOR));
		jsonObject.put("touser", openid);
		jsonObject.put("template_id", "-DHdyygXxR3CBFCVcIq8OJEGNmkIxZP8SE94n_2HSO4");
